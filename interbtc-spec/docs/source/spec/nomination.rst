...
They lock their free collateral so that trusted Vaults can issue `interbtc` backed by the nominated collateral.
...
#. Vaults may opt out of the Nomination protocol which force refunds Nominators. 
...
This means that an Vault can only withdraw collateral as long as the fraction of nominated collateral does not exceed the threshold cap.
...
As Vaults may have issued ``interbtc`` with nominated collateral when this feature is turned off,
a ``False`` value of this scalar only prevents the opting in of new Vaults.
...
* The calling account MUST be root.
